...
<script type="text/javascript">
	...
	function sum()
	{
		...
		// URL 구성
		var url = "test02.do?n1=" + n1 + "&n2=" + n2; // open()에서 쓰려고
		...
		// 『true』 : boolean → true(비동기), false(동기), 동기는 사용 안 함. // 약관 동의 처럼. 거부하면 처리 안 된다.
		ajax.open("GET", url, true); // 비동기 방식
		//-- 실제 서버와의 연결이 아니고 일종의 환경 설정 부분으로 이해하는 것이 좋다.
		//   (실제 서버 연결은 나중에...)
		
		// 우리가 직접 움직여!!! 하는 게 아니라 현관문을 지켜봐! 라고 하는 거고
		// 누군가가 들어올 때 현관문이 열리면 작동하도록
		
		// ajax 객체(XMLHttpRequest 객체)의 readyState 속성은
		// 각 단계별로 변화되며, 0부터 4까지 순차적으로 증가한다.
		// 그 때마다 ... 『readystatechange』 이벤트가 발생한다.
		ajax.onreadystatechange = function() // ajax 객체에 의해 호출 당하는 것. 우리가 호출하는 게 아님.
		{
		...
		// GET 방식으로 데이터 전송을 하는 경우... 인자값이 ""
		// POST 방식으로 데이터를 전송하는 경우... 인자값에 데이터를 넣는다.
		// 현재는 GET 방식의 요청이다. (비워두는 것을 하지말자.)
		ajax.send("");
